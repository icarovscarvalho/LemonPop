  ...
  return(
    <nav className={`flex gap-2 justify-items-center ${menuMobileButton} md:flex`}>
      <div className="flex flex-col md:flex-row gap-5">
        {menus.map( item =>
          <a href={`/${item.page}`} key={item.btn} className="py-[5px] px-[10px] hover:bg-yellow-500 transition rounded-2xl hover:shadow-md/10 hover:-translate-y-0.5 duration-300 ease-in-out">{item.btn}</a>
        )}
